...
def temporal_softmax_loss(x, y, mask):
    ...
    p = exp_x/sum_x
    loss = -np.sum(mask*np.log(p[np.arange(N*T),y]))/N
    
    indicator = np.zeros(x.shape)
    indicator[np.arange(N*T),y]=1
    dx = mask[:,None]*(p-indicator) / N
